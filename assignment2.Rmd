...
setwd("C:/Users/Adam/OneDrive/Documents/assignment-2-ad700-bit")
getwd()
...
```{r}
Partners8 <- Egoalt8 %>%
        filter(h_relationship %in% c("1","2")) %>%
...
```{r}
Stable2 <- Stable2 %>%
        name(race) <- mutate(ifelse(racel_dv <= 4, "White","Non-White")) 
```
...
```{r}
JoinedEthn <- Hetero8 %>%
  left_join(Stable2, copy = FALSE, suffix = c("pidp", "pidp"))
